...
public class LeaderClientExecutorTest {
  ...
  public void testLeaderMultipleTasks() throws LeaderTimeoutException, LeaderException {
    ...
    AssignmentManager<TaskAssignment, HostMemberIdentity> taskManager1 =
        new DefaultLeaderTasksManager<>(Map.of("task1", run(inc1, 1), "task2", run(inc2, 2)));

    AssignmentManager<TaskAssignment, HostMemberIdentity> taskManager2 =
        new DefaultLeaderTasksManager<>(Map.of("task1", run(inc3, 3), "task2", run(inc4, 4)));
    ...
    final KafkaLeaderElector<TaskAssignment, HostMemberIdentity> leader1 =
        new KafkaLeaderElector<>(
            taskManager1, protocol, properties, HostMemberIdentity.builder().host("host1").build());
    final KafkaLeaderElector<TaskAssignment, HostMemberIdentity> leader2 =
        new KafkaLeaderElector<>(
            taskManager2, protocol, properties, HostMemberIdentity.builder().host("host2").build());
    ...
    await().atMost(30, TimeUnit.SECONDS).until(() -> inc1.get() > 0 || inc3.get() > 0);

    await().atMost(30, TimeUnit.SECONDS).until(() -> inc2.get() > 0 || inc4.get() > 0);
  ...
  public void testLeader() throws LeaderTimeoutException, LeaderException {
    ...
    await().atMost(30, TimeUnit.SECONDS).until(() -> inc.get() > 0 || inc2.get() > 0);
    ...
    if (firstInc) {
      ...
      await().atMost(60, TimeUnit.SECONDS).until(() -> leader1.getRestarts() > 0);

      leader1.close();

      await().atMost(60, TimeUnit.SECONDS).until(() -> inc2.get() > inc2Value);
      ...
      await().atMost(30, TimeUnit.SECONDS).until(() -> leader2.getRestarts() > 0);

      leader2.close();

      await().atMost(30, TimeUnit.SECONDS).until(() -> inc.get() > incValue);
  ...
  public AssignmentManager<TaskAssignment, HostMemberIdentity> wrap(Task task) {
    return new DefaultLeaderTasksManager<>(Map.of("default", task));
  }
