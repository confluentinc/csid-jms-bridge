...
/**
 * Class wholly copied from ActiveMQs source for JMSTestBase to intercept and change.
 */
@SuppressWarnings({"checkstyle:ClassDataAbstractionCoupling", "deprecation"})
public class MyJmsTestBase extends ActiveMQTestBase {

  protected ActiveMQServer server;

  protected org.apache.activemq.artemis.jms.server.impl.JMSServerManagerImpl jmsServer;
  ...
  protected Queue createQueue(final boolean storeConfig, final String queueName)
      throws Exception {
  ...
  protected void createCF(final String cfName,
    ...
    ConnectionFactoryConfiguration configuration = new ConnectionFactoryConfigurationImpl()
        .setName(cfName).setConnectorNames(connectorNames).setRetryInterval(1000)
  ...
  protected final void sendMessages(JMSContext context, JMSProducer producer, Queue queue,
      final int total) {
    try {
  ...
  protected final void receiveMessages(JMSConsumer consumer, final int start,
    ...
    try {
      for (int i = start; i < msgCount; i++) {
        ...
        Assert
            .assertEquals("expected=" + i + ". Got: property['counter']=" + actual, i, actual);
        if (ack) {
          message.acknowledge();
        }
      }
